document.addEventListener("DOMContentLoaded", function () {
    ...
    const workInProgreshProject = document.getElementById("workInProgreshProject");

    // Fetch the total project count from the API
    fetch(`${API_BASE_URL}/api/project/count-wip-last-year`)
    // fetch("http://localhost:9090/api/project/count-wip-last-year")
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            workInProgreshProject.textContent = data; // Assuming the response is a plain number
        })
        .catch(error => {
            console.error("Error fetching project count:", error);
            workInProgreshProject.textContent = "Error"; // Display error message if fetch fails
        });
});
...
async function fetchProjects() {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/project/wip-projects-registered-last-year`);
        // const response = await fetch('http://localhost:9090/api/project/wip-projects-registered-last-year');
