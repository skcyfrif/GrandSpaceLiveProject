document.addEventListener("DOMContentLoaded", function () {
    ...
    // fetch("http://localhost:9090/api/project/count-not-assigned-projects-last-year")
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            confirmedProjects.textContent = data; // Assuming the response is a plain number
        })
        .catch(error => {
...
async function fetchProjects() {
    try {
        ...
        // const response = await fetch('http://localhost:9090/api/project/not-assigned-projects-registered-last-year');
