document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch(`${API_BASE_URL}/api/project/count-completed-projects-last-year`)
    // fetch("http://localhost:9090/api/project/count-completed-projects-last-year")
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            completedProjects.textContent = data; // Assuming the response is a plain number
        })
        .catch(error => {
            console.error("Error fetching project count:", error);
            completedProjects.textContent = "Error"; // Display error message if fetch fails
        });
});
...
async function fetchProjects() {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/project/completed-projects-registered-last-year`);
        // const response = await fetch('http://localhost:9090/api/project/completed-projects-registered-last-year');
