...
public class ManagerService {
	...
	@Autowired
    private MeterialsRepository materialsRepository;
...
//	}
	...
	/**
     * Adds a new material associated with a project and manager.
     * 
     * @param materialsDTO the material data transfer object
     * @return the saved material entity
     */
    public Meterials addMaterial(MaterialsDTO materialsDTO) {
        // Fetch the Project by ID
        Project project = projectRepository.findById(materialsDTO.getProjectId())
                .orElseThrow(() -> new IllegalArgumentException("Project not found with ID: " + materialsDTO.getProjectId()));

        // Fetch the Manager by ID
        Manager manager = managerRepository.findById(materialsDTO.getManagerId())
                .orElseThrow(() -> new IllegalArgumentException("Manager not found with ID: " + materialsDTO.getManagerId()));

        // Map the MaterialsDTO to Materials entity
        Meterials material = modelMapper.map(materialsDTO, Meterials.class);

        // Set the Project and Manager relationships
        material.setProject(project);
        material.setManager(manager);

        // Save the material entity to the repository
