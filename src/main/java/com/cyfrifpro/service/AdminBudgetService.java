...
public class AdminBudgetService {
    ...
    Client client = project.getClient();  // Assuming that Project has a reference to Client

    if (client == null) {
        throw new IllegalArgumentException("Client not found for the project with ID: " + projectId);
    }
    ...
    // Create the final project budget with two scenarios, including the manager and client
    ProjectBudget projectBudget = new ProjectBudget(
        project, manager, managerBudget, estimatedBudget, materialCost, profitMargin);

    // Set the client on the project budget
    projectBudget.setClient(client); // Set the client from the project
