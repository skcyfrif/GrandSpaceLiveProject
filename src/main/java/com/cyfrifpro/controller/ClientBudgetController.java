...
public class ClientBudgetController {
	 ...
	 */
	@PostMapping("/confirm/{projectBudgetId}")
	@PreAuthorize("hasRole('CLIENT')")
	public ResponseEntity<String> confirmBudgetSelection(@PathVariable Long projectBudgetId,
			@RequestBody BudgetOption selectedOption) {
		try {
			clientBudgetService.confirmBudgetSelection(projectBudgetId, selectedOption);
	...
	@GetMapping("/final-budget/{clientId}")
	public ResponseEntity<?> getFinalBudgetByClientId(@PathVariable Long clientId) {
		Optional<BigDecimal> finalBudget = clientBudgetService.getFinalBudgetByClientId(clientId);

		if (finalBudget.isPresent()) {
			return ResponseEntity.ok(finalBudget.get());
		} else {
			return ResponseEntity.badRequest().body("No budget found for the given client ID.");
		}
	}
	...
	public ResponseEntity<List<ClientBudget>> getClientBudgetsByClientId(@PathVariable Long clientId) {
		List<ClientBudget> clientBudgets = clientBudgetService.getClientBudgetsByClientId(clientId);
		return ResponseEntity.ok(clientBudgets);
	}
