...
async function showBudgetDetails(projectId) {
    try {
        ...
        const projectResponse = await fetch(`${API_BASE_URL}/api/project/projects/${projectId}/budget`);
        if (!projectResponse.ok) {
            throw new Error("Failed to fetch project budget details");
        }

        const budgetDetails = await projectResponse.json();
        const budget = budgetDetails || "Not Available";

        // Fetch final budget using clientId from sessionStorage
        const clientId = sessionStorage.getItem("userId") || "123"; // Replace "123" with fallback if needed
        const finalBudgetResponse = await fetch(`${API_BASE_URL}/api/client-budget/final-budget/${clientId}`);
        
        let finalBudget = "Not Available";
        if (finalBudgetResponse.ok) {
            finalBudget = await finalBudgetResponse.json();
        }
...
async function showImage(projectId) {
    try {
        ...
        // Fetch the image for the selected project, passing the Bearer token in the headers
